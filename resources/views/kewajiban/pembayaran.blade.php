        ...
        <div class="card stacked-2">
            <div class="card-header flex-wrap border-0 pb-0 align-items-end">
                <div class="d-flex align-items-center mb-3 mr-3">
                    ...
                    <div class="mr-auto">
                        <!-- <h5 class="fs-20 text-black font-w600">Nama</h5> -->
                        <span class="text-num text-black font-w600">{{ $siswa->nama }}</span>
                    </div>
                </div>
                <div class="mr-3 mb-3">
                    <p class="fs-14 mb-1">Kelas</p>
                    <span class="text-black">{{ $siswa->kelas->nama }}</span>
                </div>
                <div class="mr-3 mb-3">
                    <p class="fs-14 mb-1">Jurusan</p>
                    <span class="text-black">{{ $siswa->kelas->jurusan->nama }}</span>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="card-body">
                <div class="media align-items-center">
                    <div class="media-body">
                        <h2 class="fs-24 text-black font-w600">Rp. {{ number_format($siswa->kewajiban()->wherePivot('kewajiban_id', '!=', 1)->sum('biaya'), 0, ",",".") }}</h2>
    ...
    <div class="col-xl-4 col-sm-6">
        <div class="card">
            <div class="card-body">
                <div class="media align-items-center">
                    <div class="media-body">
                        <h2 class="fs-24 text-black font-w600">Rp. {{ number_format($siswa->kewajiban()->wherePivot('kewajiban_id', '!=', 1)->wherePivot('status', 'Lunas')->sum('biaya') , 0, ",",".")   }}</h2>
    ...
    <div class="col-xl-4 col-sm-6">
        <div class="card">
            <div class="card-body">
                <div class="media align-items-center">
                    <div class="media-body">
                        <h2 class="fs-24 text-black font-w600">Rp. {{ number_format($siswa->kewajiban()->wherePivot('kewajiban_id', '!=', 1)->sum('biaya') - $siswa->kewajiban()->wherePivot('status', 'Lunas')->wherePivot('kewajiban_id', '!=', 1)->sum('biaya'), 0, ",",".")   }}</h2>
    ...
    <div class="col-lg-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <h4 class="card-title">Data Kewajiban Siswa</h4>
            </div>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-responsive-md" id="example">
                        ...
                        <tbody>
                            @if(request('siswa'))
                            @foreach($siswa->kewajiban()->wherePivot('kewajiban_id', '!=', 1)->get() as $kewajiban)
                            <tr>
                                <td>{{ $loop->iteration }}</td>
                                <td>{{ $kewajiban->nama }} </td>
                                <td>Rp. {{ number_format($kewajiban->biaya, 0, ',', '.') }} </td>
                                <td>{{ $kewajiban->pivot->tgl_bayar ? Carbon\Carbon::parse($kewajiban->pivot->tgl_bayar)->format('d/m/Y') : '-' }}</td>
                                <td>
                                    <span class="badge badge-{{ $kewajiban->pivot->status == 'Lunas' ? 'success' : 'danger' }}">{{ $kewajiban->pivot->status }}</span>
                                </td>
                                <td>
                                    <a href="{{ route('kewajiban.bayar', $kewajiban->id) }}?siswa={{ $siswa->id }}" class="btn btn-primary shadow btn-xs sharp mr-1"><i class="fa fa-pencil"></i></a>
                                </td>
                            </tr>
                            @endforeach
                            @endif
